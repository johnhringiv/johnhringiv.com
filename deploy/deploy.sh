...
#
# Deploys the newest main build (:latest, published by CI on every push).
...
deploy_one() {
    ...
    # Graceful stop first (SIGTERM -> tini/start.sh clean shutdown -> exit 0) so
    # Synology's Container Manager doesn't report a "stopped unexpectedly" (which
    # a SIGKILL from `rm -f` on a running container triggers). rm -f then just
    # removes the already-stopped container.
