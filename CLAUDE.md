...
- **CI** (`.github/workflows/ci.yml`): on every push/PR, builds the image (which validates `build_db.php`, image generation, shiki) and runs the **full multi-browser E2E suite** (chromium + mobile/tablet viewports, firefox, webkit — same suite as `npm run test:docker`) against the running container, with the W3C Nu validator started as a sidecar container for the HTML-validation specs. This works because the suite tests the final container, not local build output: `css-validation` fetches stylesheets over HTTP from the container rather than reading `www/generated/` off disk. CI fetches **Git LFS** content (`lfs: true`) since images + `data/cissp_results.json` are LFS-tracked; without it the build fails parsing the cissp pointer.
